...
describe('TransactionsService', () => {
    ...
    describe('getHistory', () => {
        it('returns paginated results filtered by category', async () => {
            ...
            expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('t.category = :category', { category: 'groceries' });
        });
    ...
    describe('categorize', () => {
        ...
        it('throws ForbiddenException when the transaction belongs to another user', async () => {
            ...
            await expect(service.categorize('user-1', 'tx-1', 'groceries')).rejects.toThrow(common_1.ForbiddenException);
        });
        it('throws NotFoundException when the transaction does not exist', async () => {
            repo.findOne.mockResolvedValue(null);
            await expect(service.categorize('user-1', 'missing', 'groceries')).rejects.toThrow(common_1.NotFoundException);
        });
    });
